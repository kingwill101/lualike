...
void initializeCoroutineLibrary(Interpreter interpreter) {
  ...
  coroutineLib[Value("resume")] = Value((List<Object?> args) async {
    ...
    try {
      ...
      // Return the result to the global environment, not the coroutine's environment
      interpreter.setCurrentEnv(interpreter.globals);
      ...
      // Make sure we're using the global environment
      interpreter.setCurrentEnv(interpreter.globals);
    }
  });
  ...
  coroutineLib[Value("yield")] = Value((List<Object?> args) async {
    ...
    try {
      ...
      interpreter.setCurrentEnv(currentEnv);
      Logger.debug(
        '>>> Restored environment after yield: ${currentEnv.hashCode}',
        category: 'Coroutine',
      );
    }
  });
  ...
  coroutineLib[Value("wrap")] = Value((List<Object?> args) async {
    ...
    return Value((List<Object?> resumeArgs) async {
      ...
      try {
        ...
        return Value(null) as Value; // Return nil if no values
      } finally {

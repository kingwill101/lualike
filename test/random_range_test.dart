...
void main() {
  group('Math Random Range Tests', () {
    ...
    test('math.random should produce values close to 0 and 1', timeout: Timeout(Duration(seconds: 30)), () async {
      ...
      final foundRange = (bridge.getGlobal('found_range') as Value).raw as bool;
      ...
      expect(minVal, lessThan(0.001), 
          reason: 'Should generate values very close to 0, got $minVal');
      expect(maxVal, greaterThan(0.999), 
          reason: 'Should generate values very close to 1, got $maxVal');
      expect(foundRange, isTrue, 
          reason: 'Should find values close to both 0 and 1 within 100k iterations');
    });
    
    test('math.random distribution test simplified', timeout: Timeout(Duration(seconds: 10)), () async {
      await bridge.runCode('''
      ...
      expect(lowerHalf + upperHalf, equals(rounds), 
    ...
    test('math.random basic range test', timeout: Timeout(Duration(seconds: 10)), () async {
      ...
      final allInRange = (bridge.getGlobal('all_in_range') as Value).raw as bool;

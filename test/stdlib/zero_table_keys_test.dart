...
void main() {
  group('Zero Table Keys (Lua Interpreter)', () {
    ...
    test('negative zero and positive zero as table keys - direct assignment', () async {
    ...
    test('negative zero and positive zero as table keys - constructor syntax', () async {
      await bridge.runCode('''
        local mz = -0.0
        local z = 0.0
        local t = {[mz] = 42}
      ...
      final result = bridge.getGlobal('result') as Value;
      expect(result.raw, equals(42),
        reason: 'Constructor syntax: {[mz] = 42} should be accessible via t[z]');
    });

    test('positive zero and negative zero as table keys - constructor syntax', () async {
      await bridge.runCode('''
        local mz = -0.0
        local z = 0.0
        local t = {[z] = 100}
      ...
      final result = bridge.getGlobal('result') as Value;
      expect(result.raw, equals(100),
        reason: 'Constructor syntax: {[z] = 100} should be accessible via t[mz]');
    });

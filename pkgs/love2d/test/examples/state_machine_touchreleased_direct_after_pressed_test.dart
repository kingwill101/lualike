...
void main() {
  test('state machine direct touchreleased after touchpressed under bytecode', () async {
...
loaded()
''', scriptPath: '=[state machine direct touchreleased bootstrap]');
...
testbed = {
  currentReleasedType = type(current.touchreleased),
  currentType = type(current),
}
current.touchreleased(7, 11, 13)
testbed.released = received.released
''', scriptPath: '=[state machine direct touchreleased inspect]');
    final snapshot = runtime.unwrapGlobalTable('testbed')!;
    // ignore: avoid_print
    print(snapshot);
    expect(snapshot['currentReleasedType'], 'function');

...
''');
    ...
    test(
      'World:setCallbacks replays preSolve contact disabling on the next step',
      () async {
        final result = await _execute(lua, '''
local world = love.physics.newWorld(0, 0, false)
local bodyA = love.physics.newBody(world, 0, 0, 'static')
...
local events = {}
local disableNext = true

world:setCallbacks(
  nil,
  nil,
  function(_, _, contact)
    events[#events + 1] = {'pre', contact:isEnabled()}
    if disableNext then
  ...
  function(_, _, contact)
    events[#events + 1] = {'post', contact:isEnabled()}
  end
)
...
''');
        ...
        final firstPre = _indexedValues(events[0] as Map);
        expect(firstPre, <Object?>['pre', true]);

        final laterPreEvents = events
            .map((event) => _indexedValues(event as Map))
            .where((event) => event.first == 'pre')
            .skip(1)
            .toList(growable: false);
        expect(laterPreEvents, isNotEmpty);
        expect(
          laterPreEvents.any(
            (event) => event.length >= 2 && event[1] == false,
          ),
          isTrue,
        );
      },

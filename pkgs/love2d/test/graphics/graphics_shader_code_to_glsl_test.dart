  ...
  group('LOVE graphics shader code to GLSL parity', () {
    test('_shaderCodeToGLSL translates pixel-only LOVE shader code', () async {
      ...
      final shaderCodeToGlsl =
          await lualike.execute('return love.graphics._shaderCodeToGLSL')
              as Value;
      final values =
          (await shaderCodeToGlsl.call(<Object?>[
                false,
    ...
    test(
      '_shaderCodeToGLSL rejects mismatched shader language pragmas',
      () async {
        final lualike = LuaLike();
        final runtime = lualike.vm;
        installLove2d(runtime: runtime, host: LoveHeadlessHost());

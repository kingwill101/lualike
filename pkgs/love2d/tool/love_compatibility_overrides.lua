...
merge_fields(overrides.modules['love.filesystem'], {
  status = 'partial',
  conformance = 'smoke-tested',
  notes = 'Maps LOVE filesystem semantics onto Flutter assets plus persistent save directories. Directory mounts, save identity writes, File/FileData wrappers, LOVE enum tables for file, buffer, decoder, and node-type constants, source-parity File:getExtension support, host-provided DroppedFile wrappers, Lua package loading, and zip, tar-family, grp/qpak, hog, mvl, slb, vdf, wad, iso, and 7z archive mounts from direct filesystem paths, logical source/save-relative paths, DroppedFile, FileData, and generic Data wrappers are implemented. 7z archives whose file data and encoded headers, when present, use single-folder Copy, LZMA, or LZMA2 coder chains, single-pack-stream two-coder BCJ, IA64, PPC, ARM, ARMT, SPARC, ARM64, or Delta filter chains layered on those main methods, or upstream-compatible BCJ2 x86 branch-conversion folders are decoded in pure Dart across platforms. More advanced 7z layouts such as additional filter methods or other multi-pack-stream folders still fall back to a host `7z`/`7za`/`7zr` tool on IO platforms and remain unsupported elsewhere, so full cross-platform parity remains partial.',
})
...
merge_fields(overrides.symbols['love.filesystem.mount'], {
  status = 'partial',
  conformance = 'smoke-tested',
  notes = 'Logical directory mounts, mounted-module loading, and zip, tar-family, grp/qpak, hog, mvl, slb, vdf, wad, iso, and 7z archive mounts from direct filesystem paths, logical source/save-relative paths, host-provided DroppedFile wrappers, FileData, and generic Data wrappers are implemented. 7z archives whose file data and encoded headers, when present, use single-folder Copy, LZMA, or LZMA2 coder chains, single-pack-stream two-coder BCJ, IA64, PPC, ARM, ARMT, SPARC, ARM64, or Delta filter chains layered on those main methods, or upstream-compatible BCJ2 x86 branch-conversion folders are decoded in pure Dart across platforms. More advanced 7z layouts such as additional filter methods or other multi-pack-stream folders still fall back to a host `7z`/`7za`/`7zr` tool on IO platforms and remain unsupported elsewhere, so full cross-platform parity remains partial.',
})

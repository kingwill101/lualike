...
Future<void> _bootEntry(GameEntry entry) async {
  final host = LoveHeadlessHost(audioBackendFactory: _noopAudioBackendFactory);
  ...
  expect(filesystem.setSource(entry.entryAsset), isTrue);
  await runtime.loadConfIfPresent();
  final entryData = await filesystem.readFileData(
    'main.lua',
    filename: entry.entryAsset,
  );
...
void main() {
  ...
  final demoEntries = <GameEntry>[kDemoEntries[0], kDemoEntries[1]];

  for (final entry in demoEntries) {
    testWidgets(
      '${entry.title} asset-bundle startup works from game center entry',
      (tester) async {
        await _bootEntry(entry);
      },

...
usage() {
  cat <<'EOF'
Usage: benchmark_sprite_geometry_ab.sh --vm <vm-service-url> [options]

Alternates legacy matrix-heavy and reusable direct-affine sprite expansion
...
Options:
  ...
  --output-prefix <path>
                   ...
                   $TMPDIR/love2d-benchmark/sprite-geometry).
  --help           Show this help.
EOF
}
...
pair_count=7
sample_count=240
output_prefix="${TMPDIR:-/tmp}/love2d-benchmark/sprite-geometry"
...
for pair in $(seq 1 "$pair_count"); do
  ...
  for enabled in "${order[@]}"; do
    ...
    "$script_dir/benchmark_relic_breach.sh" \
      ...
      --direct-sprite-geometry "$enabled" \
...
for pair in $(seq 1 "$pair_count"); do
  ...
  ' "$output_prefix/$padded_pair-legacy.jsonl" \
    "$output_prefix/$padded_pair-direct.jsonl" >>"$pair_rows"
done
...
jq -s '
  ...
  (($pairs | length) / 2 | floor) as $middle |
  {
    ...
    legacyMedianP95: ($pairs | map(.legacy.p95) | sort | .[$middle]),
    directMedianP95: ($pairs | map(.direct.p95) | sort | .[$middle]),
    legacyMedianP99: ($pairs | map(.legacy.p99) | sort | .[$middle]),
    directMedianP99: ($pairs | map(.direct.p99) | sort | .[$middle]),
    medianPairedP95Delta:
      ($pairs | map(.direct.p95 - .legacy.p95) | sort | .[$middle]),
    medianPairedP99Delta:
      ($pairs | map(.direct.p99 - .legacy.p99) | sort | .[$middle]),
    medianPairedRenderP95Delta:
      ($pairs | map(.direct.renderP95 - .legacy.renderP95) | sort | .[$middle]),
    legacyFramesOver120Hz: ($pairs | map(.legacy.over120) | add),
    directFramesOver120Hz: ($pairs | map(.direct.over120) | add),
    pairs: $pairs
  }

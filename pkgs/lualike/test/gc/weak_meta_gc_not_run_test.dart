...
void main() {
  group('GC weak-values metatable on metatable', () {
    test(
      '__gc added under weak-values metatable must not run (no os.exit)',
      () async {
      ...
      ''');
        ...
        expect(
          ...
          isFalse,
          reason:
              'os.exit was called from __gc set under weak-values metatable; expected not invoked',
        );
      },
    );

    test(
      '__gc added after metatable set (no __gc at set time) must not run',
      () async {
        final bridge = LuaLike();
        await bridge.execute('''
      ...
      ''');
        ...
        expect(
          ...
          isFalse,
          reason:
              'os.exit was called but object became finalizable only after metatable set',
        );
      },
    );
  });

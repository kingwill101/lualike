...
local function GC1()
    ...
    print("GC1: second scenario setup (GC logs enabled)")
    print("GC1: switched logging category to Interpreter (safe points)")
    finish = false;
    local i = 1
    u = setmetatable({}, {
        __gc = function()
            print("second u collected")
            finish = true
        end
    })
    ...
    until finish
    print("GC1: second scenario finished; restoring GC category")
...
do
    ...
    print("line 250")
    if not _port then
        local s10 = dosteps(10)
        local s2 = dosteps(2)
        print('dosteps debug', s10, s2)
        assert(s10 < s2)
    end
    print("line 257")
...
do -- tests for string keys in weak tables
    ...
    collectgarbage()
    assert(collectgarbage("count") >= m + 2 ^ 12 and collectgarbage("count") < m + 2 ^ 13) -- one key was collected
...
do
    ...
    repeat
        for i = 1, 1000 do
            _ENV.a = {}
        end -- no collection during the loop
    until gcinfo() > 2 * x
    collectgarbage "restart"
    _ENV.a = nil
end

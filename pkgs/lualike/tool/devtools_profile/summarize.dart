...
Future<void> main(List<String> args) async {
  final parser = ArgParser()
    ..addOption(
      ...
      allowed: const ['cpu', 'heap', 'performance', 'all', 'none'],
      defaultsTo: 'cpu',
    )
    ..addOption(
      'top',
...
List<File> _resolveLatestTargets(String latest) {
  ...
  return switch (latest) {
    'cpu' => [
        if (newestWhere(_isCpuSnapshot) case final file?) file,
      ],
    'performance' => [
...
Future<void> _summarizeJson(
  ...
  switch (activeScreenId) {
    ...
    case 'performance':
      ...
      stdout.writeln(
        '  note: binary trace decoding is not implemented in this summary tool yet',
      );
    default:
...
void _summarizeCpuSnapshot(
  ...
  for (final rawEvent in events) {
    ...
    selfByFile.update(frame.displayUrl, (count) => count + 1, ifAbsent: () => 1);
...
Future<void> _summarizeHeapCsv(File file, {required int top}) async {
  ...
  if ([classIndex, libraryIndex, instancesIndex, sizeIndex].contains(-1)) {
    stdout.writeln('Heap CSV header does not match the expected DevTools export.');
    return;
  }
...
extension on Iterable<({String className, String library, int instances, int size})> {
  List<({String className, String library, int instances, int size})> sortedBySize() {
    final list = toList()
      ..sort((a, b) {
